...
Notas de robustez del bundle:

- `dataset_wide_with_target.csv` y `dataset_wide_features_zscore.csv` son obligatorios.
- `feature_means.csv` y `feature_stds.csv` son recomendables, pero si faltan se reconstruyen desde el CSV raw.
- Los `.npy` son opcionales; si faltan, el sistema sigue funcionando, solo pierde la validación exacta del split precalculado.
...
## Ejecución robusta en Colab

### Estructura esperada al descomprimir en `/content`

Si subes el `.zip` del proyecto a `/content` y lo descomprimes allí, la ejecución queda soportada de dos formas:

- usando el config raíz `example_config_attached_bundle.json`, que asume que los ficheros del bundle están en `/content`;
- usando `brent_pattern_system/example_config_attached_bundle.json`, que asume que los ficheros del bundle están un nivel por encima del paquete.

En ambos casos, los comandos pueden lanzarse con cualquiera de estas dos formas:

```bash
python -m brent_pattern_system.validate_bundle --config /content/example_config_attached_bundle.json
python brent_pattern_system/validate_bundle.py --config /content/example_config_attached_bundle.json
```

También se añade un `requirements.txt` en la raíz para que Colab pueda instalar dependencias desde `/content/requirements.txt` sin depender de rutas internas del paquete.
